...
function [meanGMVP,stddevGMVP,meanTP,stdDevTP,varpor_r,retport_r,retMeansTarget,filteredRet,filteredVar] = RiskyRiskFree (RiskyReturns,RiskFree,up,low)
%riskfree is timeseries

RF=RiskFree
%FactorsFive=FactorsFivedata(3:722,2:6)
xret=RiskyReturns-RF
[T,N] = size(RiskyReturns) ; 
...
%weights unscaled
weightsGMVP = (mvarcov^(-1))*ones(N,1)  ;
...
mm=length(retMeansTarget)
    ...
    for i=1:mm
        ...
        x_r = quadprog(mvarcov,c,[],[],Aeq_r,beq_r,vlb,vub,x0,options);
        varpor_r(i) = x_r'*mvarcov*x_r;
        weights_r(:,i) = x_r;
    end
